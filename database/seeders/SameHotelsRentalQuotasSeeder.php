...
class SameHotelsRentalQuotasSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        // Buscar os hotéis que já foram usados nas cotas anteriores dos mesmos usuários
        $existingQuotas = Quota::whereIn('user_id', [$user1->id, $user2->id])
        ...
        if (empty($existingQuotas)) {
            $this->command->error('Nenhuma cota anterior encontrada. Execute o RentalQuotasSeeder primeiro.');
            return;
        }

        $this->command->info("Encontrados " . count($existingQuotas) . " hotéis já usados. Criando novas cotas para os mesmos hotéis...");

        // Buscar os hotéis no banco - tentar correspondência exata primeiro
        $hotels = collect();
        foreach ($existingQuotas as $hotelName) {
            $hotel = Hotel::where('name', $hotelName)
            ...
            if ($hotel) {
                ...
                $hotel = Hotel::where('name', 'like', "%{$hotelName}%")
                    ->where('is_active', true)
                    ->first();
        ...
        if ($hotels->isEmpty()) {
            $this->command->error('Nenhum hotel encontrado no banco de dados.');
            return;
        }

        $this->command->info("Encontrados " . $hotels->count() . " hotéis correspondentes.");
        ...
        // Para cada mês, criar cotas nos mesmos hotéis
        foreach ($months as $monthData) {
            $month = $monthData['month'];
            $monthName = $monthData['name'];
            
            // Pegar alguns hotéis aleatórios dos já usados (2-4 por mês)
            $hotelsArray = $hotels->toArray();
            $hotelsKeys = array_keys($hotelsArray);
            shuffle($hotelsKeys);
            $hotelsForMonth = array_slice($hotelsKeys, 0, min(4, count($hotelsKeys)));
            
            foreach ($hotelsForMonth as $hotelName) {
                $hotel = $hotels->get($hotelName);
                
                // Garantir que temos um objeto Hotel válido
                if (!$hotel || !($hotel instanceof Hotel)) {
                    continue;
                }
                // Alternar entre os usuários
                $user = (rand(0, 1) == 0) ? $user1 : $user2;
                
                // Verificar se já existe cota para este hotel neste mês para este usuário
                ...
                // Se já existe, pular para o próximo hotel
                if ($existingQuota) {
                    continue;
                }
                ...
                $rentalOffer = RentalOffer::create([
                    ...
                    'quota_id' => $quota->id,
                    'hotel_id' => $hotel->id,
                    'title' => "Aluguel de Cota - {$hotelName} - {$monthName}",
                    'description' => "Oferta de aluguel de cota no {$hotelName} durante o mês de {$monthName}. " . 
                ...
                $this->command->info("  ✓ Cota criada para {$user->email} - {$hotelName} - {$monthName} ({$duration} dias)" . 
                                    ($isFractioned ? " [FRACIONADA]" : ""));
            }
        }

        $this->command->info("\n✓ {$createdQuotas} novas cotas criadas com sucesso!");
