...
<div class="container py-5">
    <section class="mb-4">
        <div class="p-4 p-lg-5 rounded-4 text-white" style="background: linear-gradient(135deg, rgba(0, 151, 57, 0.93), rgba(4, 64, 52, 0.9)); box-shadow: 0 30px 60px rgba(5, 74, 40, 0.28); position: relative; overflow: hidden;">
            <div class="row align-items-center">
                ...
                <div class="col-lg-4 text-lg-end text-center mt-4 mt-lg-0">
                    <span class="badge bg-{{ $statusColors[$transaction->status] ?? 'secondary' }} fs-6 px-4 py-2">
                        <i class="fas fa-{{ $statusIcons[$transaction->status] ?? 'info-circle' }} me-1"></i>{{ ucfirst($transaction->status) }}
                    </span>
            ...
            @if($isRental)
                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-body p-4">
                        ...
                        <p class="mb-3"><strong>Status:</strong> <span class="badge bg-{{ $transaction->payment_status === 'completed' ? 'success' : 'warning' }}">{{ ucfirst($transaction->payment_status) }}</span></p>
