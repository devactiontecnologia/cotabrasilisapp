@extends('layouts.app')
@section('title', 'Aguardando documento — Troca')
@section('content')
<style>
    ...
    .waiting-exchange-card-header,
    .waiting-exchange-card-header h5,
    .waiting-exchange-card-header i {
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow border-0 rounded-4">
                <div class="card-header bg-success text-white py-3 rounded-top-4 waiting-exchange-card-header">
                    <h5 class="mb-0 text-white"><i class="fas fa-exchange-alt me-2"></i>Aguardando documento do proprietário (troca)</h5>
                </div>
                <div class="card-body p-4">
                    ...
                    <p class="mb-4">Troca de periodo — sem valores monetários entre as partes. O prazo para essa transação expira em 6 horas. Aguarde o proprietário enviar o Termo de Autorização de Hospedagem para Terceiros/<i>Voucher</i>, preenchido com os nomes completos e CPFs informados por você, e assinado, para você tambem assinar com gov.br e devolver.<br>Você receberá alertas por <em>email</em> e <em>Whatsapp</em>.</p>
                    ...
                    <div id="doc-available-box" class="{{ ($transaction->workflow_step ?? '') === \App\Models\QuotaTransaction::WORKFLOW_DOC_AVAILABLE ? '' : 'd-none' }}">
                        ...
                        <form method="POST" action="{{ route('transactions.renter-signed.upload', $transaction) }}" enctype="multipart/form-data" class="mb-3">
                            ...
                            <button type="submit" class="btn btn-success mt-2"><i class="fas fa-upload me-1"></i>Enviar documento assinado</button>
                        </form>
                    </div>
                    ...
                    @if($transaction->is_fair_exchange ?? false)
                    <div id="go-payment-box" class="{{ ($transaction->workflow_step ?? '') === \App\Models\QuotaTransaction::WORKFLOW_AWAITING_TAX_PAYMENT ? '' : 'd-none' }}">
                        <div class="go-payment-fixed">
                            <i class="fas fa-arrow-right me-2"></i>Proprietário finalizou. <a href="{{ route('transactions.payment', $transaction) }}" class="fw-semibold text-decoration-underline" style="color: #0a3622;">Pague a taxa de êxito (troca justa) para concluir</a>
                        </div>
                    </div>
                    @endif

                    <div id="exchange-completed-box" class="{{ ($transaction->workflow_step ?? '') === \App\Models\QuotaTransaction::WORKFLOW_COMPLETED ? '' : 'd-none' }}">
                        <div class="signed-sent-fixed">
                            <i class="fas fa-check-circle me-2"></i>Troca concluída com sucesso!
                            <a href="{{ route('transactions.show', $transaction) }}" class="fw-semibold text-decoration-underline ms-2" style="color: #0a3622;">Ver detalhes</a>
                        </div>
                    </div>
